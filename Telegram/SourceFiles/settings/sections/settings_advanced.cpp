...
void FamilyGramServerBox(
		not_null<Ui::GenericBox*> box,
		not_null<Window::SessionController*> controller) {
	box->setTitle(rpl::single(u"FamilyGram server"_q));
	...
	content->add(
		object_ptr<Ui::FlatLabel>(
			...
			rpl::single(
				...
				u"Changing servers signs you out and requires signing in again."_q),
			st::boxLabel),
		st::boxRowPadding);
	...
	const auto field = content->add(
		object_ptr<Ui::InputField>(
			...
			rpl::single(u"Server address"_q),
			Local::readFamilyGramServerHost()),
		st::boxRowPadding);
	...
	const auto apply = [=] {
		const auto host = FamilyGram::NormalizeServerHost(field->getLastText());
		if (!FamilyGram::IsValidServerHost(host)) {
			field->setFocus();
			field->showError();
			error->setText(u"Enter a valid server IP address or domain name."_q);
			error->show();
			return;
		}
		error->hide();
		const auto previous = Local::readFamilyGramServerHost();
		if (host == previous) {
			box->closeBox();
			return;
		}
		...
		box->uiShow()->showBox(Ui::MakeConfirmBox({
			.text = rpl::single(
				u"Change the FamilyGram server and sign out on this device?"_q),
			.confirmed = save,
			.confirmText = rpl::single(u"Change server"_q),
			.title = rpl::single(u"Change server"_q),
		}));
	};

	box->setFocusCallback([=] { field->setFocusFast(); });
	field->submits() | rpl::on_next(apply, field->lifetime());
	box->addButton(rpl::single(u"Save"_q), apply);
	box->addButton(tr::lng_cancel(), [=] { box->closeBox(); });
}

void BuildFamilyGramServerSection(SectionBuilder &builder) {
	const auto controller = builder.controller();

	const auto serverLabel = [=] {
		const auto host = Local::readFamilyGramServerHost();
		return host.isEmpty() ? u"Not configured"_q : host;
	};

	builder.addSkip();
	builder.addSubsectionTitle({
		.id = u"advanced/familygram"_q,
		.title = rpl::single(u"FamilyGram"_q),
		.keywords = {
			u"familygram"_q,
			u"server"_q,
			u"host"_q,
	...
	builder.addButton({
		.id = u"advanced/familygram_server"_q,
		.title = rpl::single(u"Change server"_q),
		.icon = { &st::menuIconNetwork },
		.label = rpl::single(serverLabel()),
		.onClick = [=] {
			controller->show(Box(
				FamilyGramServerBox,
				controller));
...
const auto kMeta = BuildHelper({
	...
	BuildFamilyGramServerSection(builder);
