...
const QString &readAutoupdatePrefixRaw() {
	...
	return AutoupdatePrefix("https://testgramdesktop.xie.su");
}
...
QString &FamilyGramServerHostCache() {
	static auto value = QString();
	return value;
}

QString familyGramServerHostFile() {
	return cWorkingDir() + "tdata/familygram_server";
}

} // namespace

void writeFamilyGramServerHost(const QString &host) {
	const auto normalized = host.trimmed().toLower();
	if (FamilyGramServerHostCache() == normalized) {
		return;
	}
...
QString readFamilyGramServerHost() {
	auto &cached = FamilyGramServerHostCache();
	if (!cached.isEmpty()) {
		return cached;
	}
	QFile f(familyGramServerHostFile());
